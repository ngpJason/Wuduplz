...
const Profile = ({RootStore ,navigation}) => {
	...
	const [Head,setHead] = useState(RootStore.Image==""?DEFAULT_IMAGE:SERVER_ADDRESS+RootStore.Image.split('|').join('//'))
	const [NickName,setNickName] = useState(RootStore.UserName)
	const [Email,setEmail] = useState(RootStore.Email)
	const [Keyword1,setKeyword1] = useState(RootStore.keywords[0]?RootStore.keywords[0].KeywordContent:'')
	const [Keyword2,setKeyword2] = useState(RootStore.keywords[1]?RootStore.keywords[1].KeywordContent:'')
	const [Keyword3,setKeyword3] = useState(RootStore.keywords[2]?RootStore.keywords[2].KeywordContent:'')
	const [City,setCity] = useState(RootStore.City)
	const [Country,setCountry] = useState(RootStore.Country)
	const [Membership,setMembership] = useState(RootStore.membership)

	const setParameters = ()=>{
		setHead(RootStore.Image==""?DEFAULT_IMAGE:SERVER_ADDRESS+RootStore.Image.split('|').join('//'))
		setNickName(RootStore.UserName),
		setEmail(RootStore.Email)
		setKeyword1(RootStore.keywords[0]?RootStore.keywords[0].KeywordContent:'')
		setKeyword2(RootStore.keywords[1]?RootStore.keywords[1].KeywordContent:'')
		setKeyword3(RootStore.keywords[2]?RootStore.keywords[2].KeywordContent:'')
	...
	<View style={themedStyle.root}>
		...
		<Image
      	...
      	/>
			...
			<Text style={{marginTop:7,borderRadius:4,width:45,marginHorizontal:10,
				backgroundColor:Membership==0?'green':Membership==1?'#ffd700':'blue',color:'white',
				fontSize:10,height:15,textAlign:'center'}}>{RootStore.membership==0?'Free':RootStore.membership==1?'Golden':'Diamond'}</Text>
			</View>
